...
import MetricGrid from "../../../../src/components/MetricsTile/MetricsGrid/MetricGrid";
...
describe("Metric grid", () => {
  const wrapper = shallow(<MetricGrid  {...props} />);
  ...
  it("should be defined for with collapsed state", () => {
  ...
  it("should call getRowStyle", async () => {
    expect(wrapper.instance().getRowStyle(agGridParams)).toEqual({ "background": "#FFFF66" })
  })

  it("should call getRowStyle for non selected RM code", async () => {
    wrapper.setProps({ selectedRMCode: 999 });
    expect(wrapper.instance().getRowStyle(agGridParams)).toEqual({ "background": "rgb(245, 245, 245)" })
  })

  it("should call rowClassRules ", async () => {
    expect(wrapper.instance().rowClassRules().highlightRow(agGridParams)).toBe(false);
  })
